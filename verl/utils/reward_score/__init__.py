...
def val_compute_score(data_source, solution_str, ground_truth, extra_info=None):
    if data_source == 'openai/gsm8k':
        from . import gsm8k
        res = gsm8k.compute_score(solution_str, ground_truth)
    elif data_source in ['lighteval/MATH', 'DigitalLearningGmbH/MATH-lighteval', 'math-500']:
        ...
        from . import orz_verifier
        res = orz_verifier.val_compute_score(solution_str, ground_truth)
    else:
        raise NotImplementedError
...
def my_compute_score(data_source, solution_str, ground_truth, extra_info=None):
    if data_source == 'openai/gsm8k':
        ...
        from . import prime_code
        res = prime_code.compute_score(solution_str, ground_truth, continuous=True)
    elif data_source in ['orz_dataset']:
        from . import orz_verifier
        res = orz_verifier.compute_score(solution_str, ground_truth)
    else:
        raise NotImplementedError
